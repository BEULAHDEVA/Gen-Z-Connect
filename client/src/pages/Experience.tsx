              ...
              >
                ...
                <div className="grid md:grid-cols-[1fr_2fr] gap-4 md:gap-12 items-baseline">
                  <div>
                    ...
                    <div className="inline-block px-2 py-0.5 mt-2 bg-black/5 border border-black/10 text-xs font-mono text-muted-foreground uppercase">
                      {job.role}
                    </div>
                  </div>
                  <div className="relative group p-6 bg-card border border-black/5 hover:border-primary/50 transition-colors duration-300">
        ...
        <section className="space-y-12 pt-12">
          <header className="text-center md:text-left flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              ...
              <p className="font-mono text-muted-foreground mt-4 ml-1">ACCREDITED SKILLS & CREDENTIALS</p>
            </div>
            <motion.div
          ...
          <div className="relative border-l border-black/10 ml-4 md:ml-0 space-y-12">
            {experiences?.filter(exp => exp.role.toLowerCase().includes('certification')).map((cert, index) => (
              ...
              >
                ...
                <div className="grid md:grid-cols-[1fr_2fr] gap-4 md:gap-12 items-baseline">
                  <div>
                    <h3 className="font-mono text-secondary text-sm mb-1">{cert.duration}</h3>
                    <h2 className="text-2xl font-display font-bold uppercase">{cert.company}</h2>
                    <div className="inline-block px-2 py-0.5 mt-2 bg-black/5 border border-black/10 text-xs font-mono text-muted-foreground uppercase">
                      {cert.role.replace(/certification:\s*/i, '')}
                    </div>
                  </div>
                  <div className="relative group p-6 bg-card border border-black/5 hover:border-secondary/50 transition-colors duration-300">
                    <p className="relative font-mono text-sm md:text-base text-muted-foreground leading-relaxed">
                      {cert.description}
                    </p>
                  </div>
